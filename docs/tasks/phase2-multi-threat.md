...
> **이 문서는 6차에 걸친 사용자 검토를 거치며 진화했습니다.**
> 1. 초안: 단순 다중 위협 + S-S 교리 추가
> 2. 보강: 8개 충돌/완결성 이슈 식별 + 해결책
> 3. 일반화: EADSIM 철학 적용, 케이스 처방 → 일반화 재설계
> 4. AssignmentStrategy 도입 (이후 폐기)
> 5. **개념 재정의**: Linear vs Kill-web의 본질은 알고리즘이 아닌 "정보 풀의 범위와 신선도"
> 6. **Phase 2.0 자료 조사**: 공개 자료 기반 모델링 결정 (이 섹션)
>
> 새 세션이 이 문서를 읽을 때는 **§0 (Phase 2.0 자료 조사 결과)부터 읽고**,
> 이후 §1 이하 작업 명세서로 진입할 것.
