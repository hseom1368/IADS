...
3. **S-L-S/S-S 교전 교리**: Shoot-Look-Shoot(1발→BDA→재발사) / Shoot-Shoot(2발 동시)
4. **운용원 지연**: C2 노드별 숙련도(고/중/저)에 따른 판단 시간 명시적 모델링
5. **포대 단위 동시교전 상한**: MFR 동시유도 발수로 제한 (L-SAM 20/10, 천궁-II 10, PAC-3 9)
6. **밴드별 재밍 감수성**: L밴드(강건) → S → C → X밴드(취약)
7. **추적 상관 모델**: 오상관/미상관 확률 → 과잉교전/미교전 재현
...
3. ~~즉시 교전 판정~~ → **S-L-S/S-S 교리 + BDA 지연**
4. ~~운용원 없음~~ → **숙련도별 판단 시간** (15~50s)
5. ~~무제한 동시교전~~ → **MFR 동시유도 상한**
6. ~~단일 재밍 계수~~ → **밴드별 감수성** (L/S/C/X)
7. ~~요격미사일 없음~~ → PNG/CLOS 물리 비행 (시각화용)
8. ~~6개 메트릭~~ → **EADSIM MOE/MOP 10개**
